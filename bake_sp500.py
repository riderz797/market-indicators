"""
Fetch S&P 500 daily history via yfinance and output JavaScript data
with both dates and close prices for each year.

Usage:  python bake_sp500.py > sp500_baked.js
"""
import json, sys
...
def main():
    ...
    # Also output a month-tick mapping computed from the most recent complete year
    ref = years[last_complete]
    month_starts = {}
    for i, ds in enumerate(ref["dates"]):
        m = int(ds[5:7])
        if m not in month_starts:
            month_starts[m] = i + 1  # 1-based trading day
    ...
    print(f"\nBaked {len([y for y in years if y < current_year])} complete years", file=sys.stderr)
